...
- **Touch screen (iPad / phone)** — a **D-pad** appears directly below the canvas. Tap it to move, or **swipe** anywhere on the canvas (up/down/left/right) to move one tile in that direction. The D-pad only shows up when the browser reports touch support, so desktop looks unchanged.
